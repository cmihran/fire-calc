...
- **`src/engine/`** — pure functions, no React.
  ...
  - `healthcare.ts` — ACA premium tax credit. `federalPovertyLevel(householdSize, year, assumptions)`, `applicablePercentage(fplRatio)` (IRA-era curve, no 400% cliff), `computeACAPremiumAndCredit({ magi, householdSize, slcspTodayDollars, year, assumptions })`. Only active in gap years (retirementAge → 64); net premium adds to annualSpending. Opt-in via `core.acaEnabled`.
...
- **`src/components/`** — React presentation. Sidebar dashboard: ProfilePicker (dropdown to switch / create / duplicate / rename / delete profiles; Demo listed as read-only) + ScenarioPicker (list / activate / rename / duplicate / delete / color / compare toggle) + Settings (balances, contributions, SS, Roth conversions, equity comp, current home, planned home events) + Controls (rate sliders) in a sticky left sidebar, Chart + MilestoneCards + YearTable in the main area. `Settings` and `Controls` are scenario-agnostic — they take `core`/`sliders` + `onChange`; `App.tsx` wires their `onChange` to `setActiveCore`/`setActiveSliders`, and wraps them in `.is-readonly-block` when the active profile is read-only so edits are visually greyed + pointer-events blocked. ScenarioPicker accepts a `readOnly` prop that disables add/duplicate/delete/rename/color.

**State:** `useAppState` holds a persisted `{ profiles, activeProfileId }` store under `networth-predict:profiles:v1` (debounced 200ms). Each profile wraps its own `AppState` (`scenarios[]`, `activeScenarioId`, `compareIds`). A synthesized **Demo** profile (`id = '__demo__'`) is always present in the dropdown, `readOnly: true` — all mutators no-op when the active profile is read-only. Scenario activation and compare-toggles still work on Demo (view-only navigation). Migration path: legacy `networth-predict:v1` (single AppState) is wrapped into one "My Profile" on first load and then cleaned up. `migrateCore()` + `migrateAppState()` still backfill fields added after a user's localStorage was written, and still wrap the old v1 `{core, sliders}` shape into a single Baseline scenario. URL params: `?demo` activates the synthesized Demo profile (read-only); `?fresh` bypasses localStorage and loads a single editable "My Profile" with one Baseline scenario — used by tests that need a known clean state.

**Types:** All in `src/types/index.ts`. Key shapes: `AppState` (`{ scenarios, activeScenarioId, compareIds }`), `Scenario` (`{ id, name, color, core, sliders }`), `CoreConfig` (includes `filingStatus` — per-scenario, so you can compare Single vs MFJ side-by-side; `Assumptions.filingStatus` is the legacy fallback only), `Assumptions`, `SliderOverrides`, `IncomeSources` (structured income breakdown — all tax calcs consume this), `TaxResult`, `Tick`, `RothConversionPlan`, `SocialSecurityPlan`.
...
Modeled: federal progressive brackets (year-indexed from 2026 base), all 50 states + DC with per-state retirement income exclusion and SS taxability, city/local (NYC + Yonkers, user-selectable), FICA (per-earner SS wage cap + joint Additional Medicare), NIIT (frozen thresholds), LTCG stacked on ordinary (0/15/20), Roth IRA MAGI phase-out (year-indexed), HSA with triple tax advantage, Social Security (claim-age adjustment + provisional-income taxation), RMDs (SECURE 2.0 schedule), Roth conversion ladders (user-configured age windows filling to bracket target), real basis tracking on taxable account with qualified/ordinary dividend + realized-gain decomposition, equity comp (RSU vest windows + NSO/ISO/ESPP exercise events), primary-residence model (mortgage amortization, property tax, §121 exclusion on sale), itemized deduction (SALT cap + mortgage interest) vs standard deduction, AMT (Form 6251 with SALT addback when itemizing + ISO bargain preference), ACA PTC (pre-Medicare gap, IRA formula, FPL-indexed applicable-%), Rule of 55 (penalty-free Traditional for 401k of last employer if retired at 55+), Medicare Part B + IRMAA at 65+ (2-year MAGI lookback, 6-tier surcharge schedule), two-earner MFJ households (independent spouse W-2 with own SS wage cap + 401k deferral limit + Roth IRA slot, independent SS PIA/claim age; Traditional/Roth/HSA stay household buckets).

Deferred to `TODO.md`: 72(t) SEPP, QSBS, QBI, SE tax, stochastic returns, spousal/survivor SS benefits, full per-state local coverage beyond NY.
